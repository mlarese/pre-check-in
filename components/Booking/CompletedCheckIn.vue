<template class="pre-check-in">
  <v-layout>
    <v-flex
      ...
      <br>
      <span>{{ $vuetify.t (' Upon your arrival at our hotel it will be sufficient to present the reception only ') }}</span>
      <br>
      <span>{{ $vuetify.t ('booking confirmation.') }}<strong>{{ $vuetify.t ('See you soon!') }}</strong></span>
    </v-flex>
    <v-spacer/>
    <v-flex
      xs3
    ...
    <span>
      <v-btn
        color="info"
        dark>
        {{ $vuetify.t('Visit Our WebSite') }}</v-btn>
    </span>
  </v-layout>
</template>
